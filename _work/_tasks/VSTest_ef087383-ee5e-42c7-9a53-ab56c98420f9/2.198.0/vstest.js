...
function startTest() {
    try {
        ...
        try {
            ...
            utils.Helper.publishEventToCi(constants_1.AreaCodes.RUNTESTSLOCALLY, error.message, 1038, true);
            throw (error);
        }
        console.log('========================================================');
        ...
        var consolidatedCiData = {
            ...
            consoleOptionsEnabled: !utils.Helper.isNullOrWhitespace(vstestConfig.otherConsoleOptions) ? vstestConfig.otherConsoleOptions : '',
        ...
        invokeVSTest().then(function (taskResult) {
            ...
            uploadVstestDiagFile();
            utils.Helper.publishEventToCi(constants_1.AreaCodes.INVOKEVSTEST, err.message, 1002, false);
        ...
        uploadVstestDiagFile();
        utils.Helper.publishEventToCi(constants_1.AreaCodes.RUNTESTSLOCALLY, error.message, 1003, false);
...
function getVstestArguments(settingsFile, addTestCaseFilter) {
    const argsArray = [];
    ...
    if (settingsFile) {
        if (utils.Helper.pathExistsAsFile(settingsFile)) {
            ...
            if (!tl.exist(settingsFile)) {
                ...
                utils.Helper.publishEventToCi(constants_1.AreaCodes.INVALIDSETTINGSFILE, 'InvalidSettingsFile', 1004, true);
...
function updateResponseFile(argsArray, responseFile) {
    ...
    try {
        ...
        utils.Helper.publishEventToCi(constants_1.AreaCodes.UPDATERESPONSEFILE, err.message, 1017, false);
        tl.error(err);
...
function executeVstest(parallelRunSettingsFile, vsVersion, argsArray, addOtherConsoleOptions) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        try {
            var code = yield vstest.exec(execOptions);
            ...
            if (ignoreTestFailures) {
                ...
                utils.Helper.publishEventToCi(constants_1.AreaCodes.EXECUTEVSTEST, err.message, 1005, true);
                tl.error(err);
                return 1;
            }
        }
...
function getVstestTestsListInternal(vsVersion, testCaseFilter, outputFile) {
    ...
    try {
        ...
        var vstestExecutionResult = vstest.execSync({ cwd: workingDirectory });
        if (vstestExecutionResult.code != 0) {
            ...
            utils.Helper.publishEventToCi(constants_1.AreaCodes.GETVSTESTTESTSLISTINTERNAL, vstestExecutionResult.error ? vstestExecutionResult.error.message : vstestExecutionResult.stderr, 1006, false);
        ...
        utils.Helper.publishEventToCi(constants_1.AreaCodes.GETVSTESTTESTSLIST, err.message, 1027, false);
        tl.error(err);
...
function uploadFile(file) {
    try {
        ...
        utils.Helper.publishEventToCi(constants_1.AreaCodes.GETVSTESTTESTSLIST, err.message, 1029, false);
        tl.debug(err);
    }
}
...
function runVStest(settingsFile, vsVersion) {
    return __awaiter(this, void 0, void 0, function* () {
        if (!isTiaAllowed()) {
        ...
        try {
            ...
            try {
                ...
                utils.Helper.publishEventToCi(constants_1.AreaCodes.GENERATERESPONSEFILE, err.message, 1024, false);
                tl.error(err);
            ...
            if (isEmptyResponseFile(tiaConfig.responseFile)) {
                ...
                if (responseContainsNoTests(tiaConfig.responseFile)) {
                    ...
                    if (updateTestResultsOutputCode !== 0) {
                        utils.Helper.publishEventToCi(constants_1.AreaCodes.UPLOADTESTRESULTS, constants_1.ResultMessages.UPLOADTESTRESULTSRETURNED + updateTestResultsOutputCode, 1011, false);
                    ...
                    if (updateResponseFileSuccess && vstestConfig.testcaseFilter)
                        tl.debug('Ignoring TestCaseFilter in response file because Test Impact is enabled');
                    return updateResponseFileSuccess ?
...
function runVsTestAndUploadResults(settingsFile, vsVersion, isResponseFileRun, updatedResponseFile, uploadTiaResults) {
    return __awaiter(this, void 0, void 0, function* () {
        var vstestArgs;
        let testselector = new testselectorinvoker.TestSelectorInvoker();
        ...
        try {
            var vscode = yield executeVstest(settingsFile, vsVersion, vstestArgs, !isResponseFileRun);
            let updateTestResultsOutputCode;
            ...
            if (vscode !== 0 || (uploadTiaResults && !vstestConfig.rerunFailedTests && updateTestResultsOutputCode !== 0)) {
                utils.Helper.publishEventToCi(constants_1.AreaCodes.EXECUTEVSTEST, constants_1.ResultMessages.EXECUTEVSTESTRETURNED + vscode, 1010, false);
                return tl.TaskResult.Failed;
            }
            return tl.TaskResult.Succeeded;
        }
        catch (err) {
            utils.Helper.publishEventToCi(constants_1.AreaCodes.EXECUTEVSTEST, err.message, 1010, false);
            tl.error(err);
            return tl.TaskResult.Failed;
        }
    });
}
function runVsTestAndUploadResultsNonTIAMode(settingsFile, vsVersion) {
    return __awaiter(this, void 0, void 0, function* () {
        let updateResponseFileSuccess = updateResponseFile(getVstestArguments(settingsFile, true), vstestConfig.responseFile);
...
function invokeVSTest() {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            utils.Helper.publishEventToCi(constants_1.AreaCodes.TIACONFIG, err.message, 1032, false);
        ...
        if (newSettingsFile) {
            if (!utils.Helper.pathExistsAsFile(newSettingsFile)) {
                if (!tl.exist(newSettingsFile)) {
                    utils.Helper.publishEventToCi(constants_1.AreaCodes.TIACONFIG, 'InvalidSettingsFile', 1033, true);
                    throw Error((tl.loc('InvalidSettingsFile', newSettingsFile)));
                }
            }
        }
        try {
            newSettingsFile = yield settingsHelper.updateSettingsFileAsRequired(vstestConfig.settingsFile, vstestConfig.runInParallel, vstestConfig.tiaConfig, vstestConfig.vsTestVersionDetails, false, vstestConfig.overrideTestrunParameters, false, vstestConfig.codeCoverageEnabled && utils.Helper.isToolsInstallerFlow(vstestConfig));
...
function vsTestCall(newSettingsFile, vsVersion) {
    return __awaiter(this, void 0, void 0, function* () {
        return runVStest(newSettingsFile, vsVersion).then(function (code) {
            if (code !== 0) {
                utils.Helper.publishEventToCi(constants_1.AreaCodes.INVOKEVSTEST, 'RunVStest returned ' + code, 1036, false);
                return tl.TaskResult.Failed;
            }
            return tl.TaskResult.Succeeded;
        }).catch(function (err) {
            utils.Helper.publishEventToCi(constants_1.AreaCodes.INVOKEVSTEST, err.message, 1037, false);
            tl.error(err);
            return tl.TaskResult.Failed;
        });
    });
...
function publishTestResults(testResultsDirectory) {
    try {
        if (testResultsDirectory) {
            ...
            utils.Helper.publishEventToCi(constants_1.AreaCodes.PUBLISHRESULTS, 'no test directory', 1041, false);
            tl.warning(tl.loc('NoTestResultsDirectoryFound'));
        }
        return tl.TaskResult.Succeeded;
    }
    catch (err) {
        utils.Helper.publishEventToCi(constants_1.AreaCodes.PUBLISHRESULTS, err.message, 1040, false);
        tl.error(err);
        return tl.TaskResult.Failed;
    }
}
...
function responseContainsNoTests(filePath) {
    try {
        ...
        utils.Helper.publishEventToCi(constants_1.AreaCodes.RESPONSECONTAINSNOTESTS, err.message, 1023, false);
        tl.error(err);
        throw err;
    }
}
