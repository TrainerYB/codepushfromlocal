...
function initTestConfigurations(testConfiguration) {
    ...
    try {
        ...
        utils.Helper.publishEventToCi(constants_1.AreaCodes.SPECIFIEDVSVERSIONNOTFOUND, error.message, 1039, true);
        throw error;
    }
...
exports.logWarningForWER = logWarningForWER;
...
exports.isDontShowUIRegKeySet = isDontShowUIRegKeySet;
...
function getDiagnosticsConfiguration() {
...
function getToolsInstallerConfiguration() {
    const toolsInstallerConfiguration = {};
    ...
    if (amd64ProfilerProxy && amd64ProfilerProxy.length !== 0) {
        ...
        if (amd64ProfilerProxy && amd64ProfilerProxy.length !== 0) {
            ...
            utils.Helper.publishEventToCi(constants_1.AreaCodes.TOOLSINSTALLERCACHENOTFOUND, tl.loc('testImpactAndCCWontWork'), 1043, false);
            tl.warning(tl.loc('testImpactAndCCWontWork'));
        }
        utils.Helper.publishEventToCi(constants_1.AreaCodes.TOOLSINSTALLERCACHENOTFOUND, tl.loc('testImpactAndCCWontWork'), 1042, false);
        tl.warning(tl.loc('testImpactAndCCWontWork'));
    }
    ...
    if (x86ProfilerProxy && x86ProfilerProxy.length !== 0) {
        ...
        utils.Helper.publishEventToCi(constants_1.AreaCodes.TOOLSINSTALLERCACHENOTFOUND, tl.loc('testImpactAndCCWontWork'), 1044, false);
        tl.warning(tl.loc('testImpactAndCCWontWork'));
    }
    return toolsInstallerConfiguration;
}
function getDistributionBatchSize(dtaTestConfiguration) {
